...
function get_list($write_row, $board, $skin_url, $subject_len=40, $bbsPath=G5_BBS_URL)
{
    global $g5, $config;
    global $qstr, $page;
    ...
    // 분류명 링크
    $list['ca_name_href'] = $bbsPath.'/board.php?bo_table='.$board['bo_table'].'&amp;sca='.urlencode($list['ca_name']);

    $list['href'] = $bbsPath.'/board.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].$qstr;
    ...
    for ($i=1; $i<=G5_LINK_COUNT; $i++) {
        ...
        $list['link_href'][$i] = $bbsPath.'/link.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].'&amp;no='.$i.$qstr;
